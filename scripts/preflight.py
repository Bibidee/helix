...
ast.parse(source)
required = ["class Helix", "run_nondet_unsafe", "def fetch_verified", "hashlib.sha256", "challenge_bond_held", "challenge_round_completed", "finalized = action.challenge_round_completed", "settle_expired_challenge", "def is_actionable", "def canonical_hash(value)", '"version": "0.3.0"', "return verdict(left) == verdict(right)", "occurrence_nonce", "commitments", "MAX_OPEN_ACTIONS_PER_DELEGATION", "open_action_count", "capacity_released", "consumer: Address", "Consumer only", "MIN_WINDOW", "300, 30 * 24 * 60 * 60", "self.actions.get(action_id) is not None"]
...
for command in ([sys.executable, "-m", "pytest", "tests/direct", "-q"], [linter, "check", str(CONTRACTS[0]), "--json"], [linter, "schema", str(CONTRACTS[0]), "--output", str(artifacts / "helix.abi.json")]):
